...
for background in ['sky','building','road','forest']:
    filedir = r''
    dstdir = r''
    num=len(os.listdir(dstdir))
    # num=0
    for name in os.listdir(filedir):
        filepath=os.path.join(filedir,name)
        dstpath=os.path.join(dstdir,'feature'+str(int(name)+num))
        input_data = torch.from_numpy(read_hyper1(filepath).reshape((1, 21, 1, 256, 320)))
        multispec_feature, state_10 = conv_lstm(input_data)
        multispec_feature = multispec_feature[0].squeeze(0).detach().numpy()
