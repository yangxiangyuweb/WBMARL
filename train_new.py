...
def get_args():
    ...
    parser.add_argument("--bandState_dim", type=int, default=21)###42/21
...
class make_Env:
    def __init__(self, bandState_dim, widthSelect_input_dim, selected_bands_num, filepath):
        ...
        self.Entropy_total = np.load('./feature_data/allEntropy.npy')
        self.IG_total = np.load('./feature_data/allIG.npy')
        self.Detaction_total = np.load('./feature_data/detact.npy')
        self.Lusudeng_total = np.load('./feature_data/penzui.npy')
        self.Noise_total = np.load('./feature_data/noise.npy')
        self.data_num = self.Entropy_total.shape[0]
        self.name_list=[]
        for name in range(self.data_num):
            if os.path.exists('./feature_data/feature' + str(name + 1) + '.npy')==True:
                self.name_list.append(name+1)
    ...
    def reset(self):
        self.reward = 0
        self.count = 0
        self.data_index = self.name_list[random.randint(0, len(self.name_list)-1)]
        self.filename = self.filepath + '/feature' + str(self.data_index) + '.npy'
        # 21, 5, 256, 320 # 42, 5, 256, 320
