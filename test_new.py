...
def predict(SFEM_model_path,wavelength_actor_model_path,bandwidth_actor_model_path,filepath):
    ...
    conv_lstm.load_state_dict(torch.load(convlstm_model_path))
    ...
    band_actor.load_state_dict(torch.load(wavelength_actor_model_path))
    width_actor.load_state_dict(torch.load(bandwidth_actor_model_path))

    images=read_hyper1(filepath)
    start_time3 = time.time()
    input_data = torch.from_numpy(images.reshape((1, 21, 1, 256, 320)))
    ...
    time_consume1 = time.time()-start_time
    time_consume2 = time.time() - start_time3

    print(wavelengths_selected_index)
    print(wavelengths_selected)
    print(bandwidths_selected)
    return images,wavelengths_selected_index, wavelengths_selected, bandwidths_selected,[time_consume1,time_consume2]


if __name__ == '__main__':
    convlstm_model_path= './pth_save/conv_lstm_multy.pth'
    wavelength_actor_model_path = './pth_save/band.pth'
    bandwidth_actor_model_path = './pth_save/width.pth'
    filepath = r'.\data\1'

    images,wavelengths_selected_index, wavelengths_selected, bandwidths_selected,[time_consume1,time_consume2]=(
        predict(convlstm_model_path,wavelength_actor_model_path,bandwidth_actor_model_path,filepath))
